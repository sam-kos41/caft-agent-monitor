...
.itdef{background:#fbf3e6;border:1px solid var(--line);
border-left:4px solid var(--dusk);border-radius:8px;
padding:12px 15px;margin:0 0 18px;max-width:780px;font-size:13px;
color:var(--ink)}
.itdef b{color:var(--dusk)}
...
<div class="itdef"><b>What “IT” means here.</b> IT =
<b>information-theoretic structure of the action sequence</b>: the
statistical shape of <i>what the agent did and in what order</i> —
how predictable each action is given the ones before it (mutual
information), how repetitive / compressible the sequence is
(compression ratio), how far its action mix drifts (KL divergence),
and how varied it is (entropy). It is computed from the <i>sequence
of action types alone</i> — no reasoning text, no outcome.
<b>Why so much reduces to it:</b> workload, perception,
error-recovery and thought–action coherence are each, ultimately,
patterns <i>in that same action sequence</i>. Once its
information-theoretic structure is in the model, each added construct
carries little the sequence had not already encoded — so each
predicts the outcome alone yet adds ≈0 <i>beyond</i> IT.</div>
...
<div class="legend">
<span><b>IT — information-theoretic action-sequence structure</b>
(load-bearing) — ● new action type · ↻ repeat (redundancy = what
compression / MI score)</span>
<span><b>Workload</b> — character count of reasoning (Sweller / Wickens)</span>
<span><b>Perception</b> — ✓ read file first · ✗ edited unread file
(Endsley L1)</span>
<span><b>Error / recovery</b> — ✗ errored · ✗→↻ errored then changed
strategy (Reason)</span>
<span><b>Thought–Action</b> — ● intent matches · ◐ partial · ○ no match
(agent-native — no HF port; the deliberate contrast case)</span>
</div>
